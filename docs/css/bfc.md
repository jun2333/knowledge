...
在理解 BFC 之前，需要先了解 CSS 盒子模型（Box Model）。

页面中的每个元素都会被渲染为一个**矩形盒子**，这个盒子由四层组成（由内到外）：
...
| 层级 | 说明 |
|------|------|
| **content** | 内容区域，显示文本、子元素等 |
| **padding** | 内边距，内容与边框之间的透明空间 |
| **border** | 边框，围绕 padding 的可见（或不可见）边界 |
| **margin** | 外边距，元素与其他元素之间的透明空间 |

### 两种盒子模型

`box-sizing` 属性决定了 `width` / `height` 的计算范围：

| 模型 | `box-sizing` | `width` 包含 | 特点 |
|------|-------------|-------------|------|
| **标准盒子模型** | `content-box`（默认） | 仅 content | 设置 padding/border 会使元素实际占据的空间变大 |
| **替代盒子模型** | `border-box` | content + padding + border | 设置的 `width` 就是元素最终占据的宽度，更直观 |

```css
/* 标准模型：实际宽度 = width + padding + border */
.standard {
  box-sizing: content-box;
  width: 200px;
  padding: 20px;
  border: 1px solid #333;
  /* 实际占据 200 + 20×2 + 1×2 = 242px */
}
...
.border-box {
  ...
  padding: 20px;
  border: 1px solid #333;
  /* 实际占据 200px，内容区被压缩为 158px */
}
```

::: tip 与 BFC 的关系
BFC 中的 **"块"（Block）** 指的就是这些块级盒子。BFC 本质上定义了一个区域，决定其中的**块级盒子**如何排列、如何相互作用。理解了盒子模型，才能理解 BFC 中 margin 重叠、高度计算等行为的含义。
:::
