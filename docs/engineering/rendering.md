...
| 维度 | CSR | SSR | SSG | ISR | DPR |
|------|-----|-----|-----|-----|-----|
| 首屏速度 | 慢 | 快 | 快 | 快 | 快（已缓存）/ 中（未缓存） |
| SEO | 差 | 好 | 好 | 好 | 好 |
| 服务器成本 | 低（静态资源） | 高（需实时计算） | 低（CDN） | 低（CDN + 按需构建） | 中（CDN + Builder 按需渲染） |
| 内容实时性 | 高（客户端请求） | 高（每次实时渲染） | 低（构建时确定） | 中（缓存 + 异步更新） | 高（缓存 + 实时渲染最新） |
| 构建时间 | 快 | 慢（每次请求） | 慢（全量构建） | 快（按需构建） | 快（按需构建，无需预构建） |
| 部署方式 | 静态资源/CDN | Node 服务器 | CDN | CDN + Builder | CDN + On-demand Builder |
| 适用场景 | 后台、工具类 | 动态内容、SEO 要求高 | 内容固定、页面少 | 页面多、更新不频繁 | 海量页面、更新频繁 |
