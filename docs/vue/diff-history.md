...
react是采取上述diff算法，由于react current fiberNode是树结构，jsx通过React.createElement生成的是数组结构，因此无法采取双端diff优化
值得一提的是，Vue 在diff过程中就完成了对DOM的操作，而React只是标记，具体DOM操作发生在commit的Mutation阶段
...
双指针遍历对比单向遍历的优势？？比如：a,b,c,d变成d,a,b,c这种情况，单向遍历需要移动三次节点，双端算法只需要移动节点d到c后面
