...
VDOM（Virtual DOM）是 React 的核心抽象层，主要作用包括：

1. **跨平台能力** — VDOM 是平台无关的 UI 描述，通过抽象渲染层适配不同的宿主环境（DOM、Canvas、React Native、服务端等），实现"一次编写，多端运行"

2. **优化 Diff 性能** — VDOM 诞生的核心目的是用 JavaScript 对象描述 UI，避免直接访问 DOM 进行 Diff 对比：
   - DOM 操作慢，直接遍历 DOM 树做对比性能差
   - DOM 对象无法承载 React 特有的元数据（props、state、effectTag 等）
   - VDOM 作为纯 JS 对象，可以在内存中高效地进行 Diff 计算，最后将最小变化批量提交到真实 DOM
...
- **优化 Diff 性能** — 操作 JS 对象进行计算远比读取 DOM 进行对比高效。Diff 算法通过对比前后两棵 VDOM 树找到变化点，VDOM 作为纯 JS 对象让 Diff 计算更高效
...
| 粒度 | 说明 | 代表 |
|------|------|------|
| **应用级** | 状态变化从 root 开始协调，通过 bailout 优化跳过未变化组件 | React |
| **组件级** | 状态变化只触发依赖该状态的组件更新，不依赖运行时优化 | Vue |
| **元素级** | 编译时精确到 DOM 节点，无需运行时协调 | Svelte、Solid |

> **React 的特殊性**：架构上是应用级（从 root 开始 DFS 协调），但通过 bailout 优化可以实现组件级的实际效果。效果稳定性取决于开发者的组件组织方式（状态局部化、组件拆分合理）。
