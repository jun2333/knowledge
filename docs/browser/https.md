...
#### ECDHE 凭什么能"独立算出同一个共享密钥"

**油漆类比**：双方先约定一种公共颜色（基点 G），各自混入秘密色（私钥 a / b）后把混合色公开（公钥 A / B）；拿对方的混合色再混入自己的秘密色，得到的颜色必然一样——因为**混合顺序无关**；而混合不可逆，别人无法从混合色分离出秘密色。

换成椭圆曲线数学：

```
Alice：私钥 a → 公钥 A = a·G（公开）
Bob：  私钥 b → 公钥 B = b·G（公开）

Alice 算：a·B = a·(b·G) = (a×b)·G
Bob   算：b·A = b·(a·G) = (b×a)·G
→ 结果相同（数乘满足交换律），这个共享值就是"共享密钥"
```

中间人只知道 G、A、B，反推 a 需要解**椭圆曲线离散对数问题**（目前无有效算法），所以共享密钥只有双方能算出来——这就是 ECDHE 不需要传输密钥本身、却能安全协商出密钥的原因。
