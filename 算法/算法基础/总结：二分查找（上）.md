...
解答：根据x的值，判断求解值y的取值范围。假设求解值范围min < y < max。若0<x<1，则min=x，max=1；若x=1，则y=1；x>1，则min=1，max=x；在确定了求解范围之后，利用二分法在求解值

```javascript
function sqrt(x) {
    // 边界处理
    if (x < 0) throw Error(`${x} is invalid number`);
    if (x === 0 || x === 1) return x;
    // 范围确定
    let min, max;
    if (x < 1) {
    ...
    // 二分法查找
    return handleNumber(binarySearch(min, max, x));
}
function binarySearch(low, high, value) {
    let mid = low + (high - low) / 2;// 此处不用位运算，因为位运算无法得出小数
    if (Math.abs(mid * mid - value) < 0.000001) return mid;
    else if (mid * mid > value) return binarySearch(low, mid, value);
    else return binarySearch(mid, high, value);
}
// 保留6位小数
// 此处不用Number和String方法
function handleNumber(num) {
    let numToStr = num + '';
    let numToStrArr = numToStr.split('.');
    if (!numToStrArr[1]) return num;
    let str = '';
    if (numToStrArr[1].length < 6) {
        let addLen = 6 - numToStrArr[1].length;
        for (let i = 0; i < addLen; i++) {
            str += '0';
        }
        str = numToStrArr[1] + str;
    } else {
        str = numToStrArr[1].substr(0, 6);
    }
    return numToStrArr[0] + '.' + str - 0;
}
```
