...
**总结来看就是：mount阶段只有HostRootFiber有flag，其他子fiber都没有，也是性能优化考量，离屏组装好dom tree之后一次挂载**
### mount
...
diffProperties(属性更新):
两次遍历：
1. 旧节点存在，新节点不存在的(已删除)----遍历旧属性(标记删除)
2. 新旧节点都存在的属性(修改)----遍历新属性(标记更新)
最后将属性存在fiberNode.updateQueue中，updateQueue是一个数组~

### flags冒泡(使用位运算，简化代码提升性能)
